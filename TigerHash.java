 ...
 * @Version： 1.0
 * @Problem：TigerHash算法生成消息认证码
 * 本程序只对512位的消息进行处理也就是64个字符的字符串
 */

//Tiger Hash总共要进行三大轮转换，每一个大轮中包括8个小轮
public class TigerHash {
    //消息
    BigInteger X;
    //对a,b,c赋初值
    BigInteger a = new BigInteger("123456789ABCDEF" , 16);
    BigInteger b = new BigInteger("FEDCBA9876543210" , 16);
    BigInteger c = new BigInteger("F096A5B4C3B2E187" , 16);
    //w[i]用来存储64位消息所对应的数值，总共512bit
    BigInteger []w = new BigInteger[8];
    //mod用来存储2^64，用来取模
    final BigInteger mod = new BigInteger("FFFFFFFFFFFFFFFF" , 16);
    //存储S盒
    final String[] table;
    //保存aa ,bb ,cc，用来求最后的a,b,值
     BigInteger aa = a;
     BigInteger bb = b;
     BigInteger cc = c;

    public TigerHash(String X){
        //将X拆分为8份，每份64个字节，8个字符
        String []x = new String[8];
        //reverse()的作用是将每份的8个字符逆序，按照小端模式存储,比如ABCDEFGH->HGFEDCBA
        for(int i=0;i < 8;++i){
            x[i] = new StringBuffer(X.substring(i*8,i*8+8)).reverse().toString();
        }
        //将x中的每份字符串按照ascll码值转换，用16进制表示，比如HGFEDCBA -> 4847464544434241
        // 再将4847464544434241 转换成BigInteger类型存储到w中
        for(int i=0;i < 8;++i){
            StringBuffer sb = new StringBuffer();
            for(int j = 0;j < 8;++j){
                sb.append(Integer.toString(x[i].charAt(j), 16));
            }

            x[i] = sb.toString();
            w[i] = new BigInteger(x[i] , 16);
        }
    ...
    //b = b * m
    //函数形参x就是w[i]
    public void round(BigInteger a , BigInteger b ,BigInteger c , BigInteger x,int m){
        ...
        for(int i=0;i < 8;++i){
            C[i] = c.shiftRight(i*8).and(new BigInteger("FF",16));
        }
        //将获得的C[i]用来计算S0[c0],S1[c2]......
        BigInteger []S = new BigInteger[8];
        //map表示对应关系 c0 -> s0  c1 -> s3 c2-> s1....c7 -> s0
        int []map = new int[]{0,3,1,2,2,1,3,0};
        for(int i=0;i < 8;++i){
            S[i] = new BigInteger(table[256*map[i]+C[i].intValue()],16);
        }

        a = a.subtract(S[0].xor(S[2]).xor(S[4]).xor(S[6]));
        b = b.add(S[1].xor(S[3]).xor(S[5]).xor(S[7]));
    ...
    //每一大轮的运算，总共8小轮，调用8次round函数，
    // 中间有几次调用需要改变a,b,c的顺序，但同样round函数执行完成后也要恢复a,b,c的顺序
    public void functionM(int m){
        round(this.a,this.b,this.c,w[0],m);
        round(this.b,this.c,this.a,w[1],m);
        BigInteger tmp = this.b;
        this.b = this.a;
        this.a = this.c;
        this.c = tmp;
        round(this.c,this.a,this.b,w[2],m);
        tmp = this.c;
        this.c = this.a;
        this.a = this.b;
        this.b = tmp;
        round(this.a,this.b,this.c,w[3],m);
        round(this.b,this.c,this.a,w[4],m);
        tmp = this.b;
        this.b = this.a;
        this.a = this.c;
        this.c = tmp;
        round(this.c,this.a,this.b,w[5],m);
        tmp = this.c;
        this.c = this.a;
        this.a = this.b;
        this.b = tmp;
        round(this.a,this.b,this.c,w[6],m);
        round(this.b,this.c,this.a,w[7],m);
        tmp = this.b;
        this.b = this.a;
        this.a = this.c;
        this.c = tmp;
    ...
    public void keySchedule(BigInteger []w){
        w[0] = w[0].subtract(w[7].xor(new BigInteger("A5A5A5A5A5A5A5A5",16)));
        w[0] = w[0].and(mod);
        w[1] = w[1].xor(w[0]);
        w[1] = w[1].and(mod);
        w[2] = w[2].add(w[1]);
        w[2] = w[2].and(mod);
        w[3] = w[3].subtract(w[2].xor((w[1].not().shiftLeft(19))));
        w[3] = w[3].and(mod);
        w[4] = w[4].xor(w[3]);
        w[4] = w[4].and(mod);
        w[5] = w[5].add(w[4]);
        ...
        w[6] = w[6].subtract(w[5].xor((w[4].not().and(mod).shiftRight(23).and(mod))));
        w[6] = w[6].and(mod);
        w[7] = w[7].xor(w[6]);
        w[7] = w[7].and(mod);
        ...
        w[0] = w[0].add(w[7]);
        w[0] = w[0].and(mod);
        w[1] = w[1].subtract(w[0].xor((w[7].not().shiftLeft(19))));
        w[1] = w[1].and(mod);
        w[2] = w[2].xor(w[1]);
        w[2] = w[2].and(mod);
        w[3] = w[3].add(w[2]);
        w[3] = w[3].and(mod);
        w[4] = w[4].subtract(w[3].xor((w[2].not().and(mod).shiftRight(23).and(mod))));
        w[4] = w[4].and(mod);
        w[5] = w[5].xor(w[4]);
        w[5] = w[5].and(mod);
        w[6] = w[6].add(w[5]);
        w[6] = w[6].and(mod);
        w[7] = w[7].subtract(w[6].xor(new BigInteger("123456789ABCDEF",16)));
        w[7] = w[7].and(mod);
        this.w = w;
    }

    //得到三大轮变换之后的a,b,c，同样也需要对每个大轮运算过后的a，b,c进行三者之间的交换，
    // 最后三大轮过后，需要对a,b,c与aa,bb,cc进行简单运算
    public void getMac(){
        aa = a;
        bb = b;
        cc = c;
        ...
        this.a = this.a.xor(aa).and(mod);
        this.b = this.b.subtract(bb).and(mod);
        this.c = this.c.add(cc).and(mod);

    }
    //对经过三大轮运算过后的a,b,c还要与[1,0,0,0,0,0,0,512]这个数组进行三大轮的运算，最终得到192bit的结果
    // 为了确保得到的a，b，c是64位的，需要对不足64位的前补0
    public void getFinalMac(){
        getMac();
        this.w[0] = new BigInteger("1",10);
        this.w[1] = new BigInteger("0",10);
        this.w[2] = new BigInteger("0",10);
        this.w[3] = new BigInteger("0",10);
        this.w[4] = new BigInteger("0",10);
        this.w[5] = new BigInteger("0",10);
        this.w[6] = new BigInteger("0",10);
        this.w[7] = new BigInteger("512",10);
        getMac();
        StringBuffer finalA = new StringBuffer(this.a.toString(16));
        StringBuffer finalB = new StringBuffer(this.b.toString(16));
        StringBuffer finalC = new StringBuffer(this.c.toString(16));

        while(finalA.length() < 16){
            finalA.insert(0,'0');
        }
        while(finalB.length() < 16){
            finalB.insert(0,'0');
        }
        while(finalC.length() < 16){
            finalC.insert(0,'0');
        }
